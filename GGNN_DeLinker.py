...
import random
import utils
from utils import dataset_info, ThreadedIterator, graph_to_adj_mat, SMALL_NUMBER, LARGE_NUMBER, graph_to_adj_mat
...
class ChemModel(object):
    ...
    def load_data(self, file_name, is_training_data: bool):
        ...
        for g in data:
            self.max_num_vertices = max(self.max_num_vertices, max([v for e in g['graph_in'] for v in [e[0], e[2]]]), max([v for e in g['graph_out'] for v in [e[0], e[2]]]))
    ...
    def make_model(self):
        ...
        with tf.variable_scope("graph_model"):
            ...
            initial_state_in = self.get_node_embedding_state(self.placeholders['initial_node_representation_in'], source=True)
            initial_state_out = self.get_node_embedding_state(self.placeholders['initial_node_representation_out'], source=False)

            # This does the actual graph work:
            if self.params['use_graph']:
                if self.params["residual_connection_on"]:
                    self.ops['final_node_representations_in'] = self.compute_final_node_representations_with_residual(
                                                                initial_state_in, 
                                                                tf.transpose(self.placeholders['adjacency_matrix_in'], [1, 0, 2, 3]),
                                                                "_encoder")
                    self.ops['final_node_representations_out'] = self.compute_final_node_representations_with_residual(
                                                                initial_state_out,
                                                                tf.transpose(self.placeholders['adjacency_matrix_out'], [1, 0, 2, 3]),
                                                                "_encoder")

                else:
                    self.ops['final_node_representations_in'] = self.compute_final_node_representations_without_residual(
                                                                initial_state_in, 
                                                                tf.transpose(self.placeholders['adjacency_matrix_in'], [1, 0, 2, 3]), self.weights['edge_weights_encoder'], 
                                                                self.weights['edge_biases_encoder'], self.weights['node_gru_encoder'], "gru_scope_encoder")
                    self.ops['final_node_representations_out'] = self.compute_final_node_representations_without_residual(
                                                                initial_state_out,
                                                                tf.transpose(self.placeholders['adjacency_matrix_out'], [1, 0, 2, 3]), self.weights['edge_weights_encoder'],
                                                                self.weights['edge_biases_encoder'], self.weights['node_gru_encoder'], "gru_scope_encoder")
        ...
        self.ops['mean'], self.ops['logvariance'] , self.ops['mean_out'], self.ops['logvariance_out'] = self.compute_mean_and_logvariance()
    ...
    def run_epoch(self, epoch_name: str, epoch_num, data, is_training: bool):
        ...
        for step, batch_data in enumerate(batch_iterator):
            ...
            batch_data[self.placeholders['z_prior_in']] = utils.generate_std_normal(\
                self.params['batch_size'], batch_data[self.placeholders['num_vertices']],self.params['hidden_size']) # FI - two noise vectors
            ...
            print("Running %s, batch %i (has %i graphs). Loss so far: %.4f. Edge loss: %.4f, KL loss: %.4f, Node symbol loss: %.4f" % (epoch_name,
                                                                                   step, num_graphs, loss / processed_graphs, 
                                                                                   edge_loss / processed_graphs, kl_loss / processed_graphs, node_symbol_loss / processed_graphs), end='\r')
        ...
        with self.graph.as_default():
            for epoch in range(1, self.params['num_epochs'] + 1):
                if not self.params['generation']:
                    ...
                    train_losses, train_speed = self.run_epoch("epoch %i (training)" % epoch, epoch,
                                                                                     self.train_data, True)
                    print("\r\x1b[K Train: Total loss: %.4f | Edge loss: %.2f, KL loss: %.2f, Node symbol loss: %.2f | instances/sec: %.2f" 
                            % (train_losses[0], train_losses[1], train_losses[2], train_losses[3], train_speed))
                    
                    valid_losses, valid_speed = self.run_epoch("epoch %i (validation)" % epoch, epoch,
                                                                                     self.valid_data, False)

                    print("\r\x1b[K Valid: Total loss: %.4f | Edge loss: %.2f, KL loss: %.2f, Node symbol loss: %.2f | instances/sec: %.2f" 
                            % (valid_losses[0], valid_losses[1], valid_losses[2], valid_losses[3], valid_speed))
                    ...
                    log_entry = {
                        'epoch': epoch,
                        'time': epoch_time,
                        'results_format': "Total loss, Edge loss, KL loss, Node symbol loss, instances/sec",
